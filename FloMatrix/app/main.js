...
import { InteractionEngine } from "./interactions/interaction-engine.js"; // <-- fixed path
...
function applyTimeframeStateToUI() {
  ...
  if (tfSpan) tfSpan.textContent = "paper"; // still show paper mode
...
function bootstrap() {
  ...
  if (overlayCanvas && floEngine && scaleEngine) {
    drawingEngine = new DrawingEngine(overlayCanvas, floEngine, scaleEngine);
  }

  if (chartCanvas && floEngine && scaleEngine) {
